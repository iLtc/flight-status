...
Expected: `wrote tests/fixtures/sfo-curated.json with N records` (N ≈ 20–30; the script throws if any curated case is missing). If a `pick()` throws, inspect the sample with `jq` and adjust the predicate — the spec's fixture table is the source of truth for *what* must be covered.
